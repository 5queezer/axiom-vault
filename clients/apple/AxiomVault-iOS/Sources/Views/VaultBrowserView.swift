...
struct VaultBrowserView: View {
    ...
    private var currentFolderName: String {
        vaultManager.currentPath == "/"
            ? "Root"
            : (vaultManager.currentPath as NSString).lastPathComponent
    }
    ...
    private var entryList: some View {
        List {
            if vaultManager.currentPath != "/" {
                Button(action: {
                    Task { await vaultManager.navigateTo(directory: "..") }
                }) {
                    HStack {
                ...
                Section {
                    Button(action: {
                        ...
                        if vaultManager.isLoading {
                            ...
                            Text("Create Folder").frame(maxWidth: .infinity)
                        }
                    }
            ...
            List {
                if let info = vaultManager.vaultInfo {
                    ...
                    Section(header: Text("Statistics")) {
                        ...
                        LabeledContent("Total Size", value: ByteCountFormatter.string(fromByteCount: info.totalSize, countStyle: .file))
                    }
                } else {
                ...
                Section {
                    Button(action: {
                        Task {
                            ...
                            if vaultManager.errorMessage == nil { dismiss() }
                        }
                    }) {
                        if vaultManager.isLoading {
                            ProgressView().frame(maxWidth: .infinity)
